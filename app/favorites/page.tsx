    ...
    <div className="min-h-screen bg-zinc-950 text-zinc-50">
      <div className="mx-auto w-full max-w-7xl px-4 py-10">
        <div className="mb-8 flex items-end justify-between">
          ...
          <Link
            href="/"
            className="rounded-full bg-white/10 px-4 py-2 text-sm text-white ring-1 ring-white/10 hover:bg-white/20"
          >
            Voltar para Home
          </Link>
        </div>
            ...
            {favorites.map((f) => (
              <div key={`${f.id}-${f.media_type}`} className="group relative">
                ...
                <button
                  ...
                  className="absolute right-2 top-2 rounded-full bg-black/50 px-2 py-1 text-xs text-white ring-1 ring-white/10 backdrop-blur hover:bg-black/60"
                >
                  Remover
                </button>
              </div>
