...
function TVDetailInner({ id }: { id: string }) {
  ...
  if (loading) return null; // Pode adicionar um componente de loading aqui se preferir
                ...
                <div className="lg:col-span-2">
                    ...
                    <div className="space-y-2 max-h-[60vh] overflow-y-auto pr-2 scrollbar-clean">
                        {episodes.map((ep) => (
                            <Link
                                key={ep.id}
                                href={`/embed/tv/${id}/${selectedSeason}/${ep.episode_number}`}
                                target="_blank"
                                className="w-full text-left p-3 rounded-lg flex items-center gap-4 transition-colors bg-zinc-900 hover:bg-zinc-800"
                            >
